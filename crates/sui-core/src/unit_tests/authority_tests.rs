...
async fn test_handle_transfer_transaction_with_max_sequence_number() {
    ...
    let recipient = dbg_addr(2);
    let authority_state =
        init_state_with_ids_and_versions(vec![(sender, object_id, SequenceNumber::MAX), (sender, gas_object_id, SequenceNumber::new())]).await;
    let object = authority_state
        .get_object(&object_id)
        .await
        .unwrap()
    ...
    .await;
    assert!(res.is_err());
    assert_eq!(res.err(), Some(SuiError::LockErrors {
...
async fn test_handle_shared_object_with_max_sequence_number() {
    ...
    assert_eq!(response.err(), Some(SuiError::LockErrors {
        errors: vec![SuiError::InvalidSequenceNumber],
    }));
...
#[cfg(test)]
pub async fn update_state_with_object_id_and_version(state: AuthorityState, address: SuiAddress, object_id: ObjectID, version: SequenceNumber) -> AuthorityState {
    let obj = Object::with_id_owner_version_for_testing(object_id, version, address);
    state.insert_genesis_object(obj).await;
    state
}
